...
<jsp:useBean id="dao" class="my.util.adminRESTAURANTDao"/>
...
String dir = "admin/image/RestaurantImage";			//저장될 경로 지정
...
try{ 
   ...
   MultipartRequest m = new MultipartRequest(request, dir, max, "utf-8", new DefaultFileRenamePolicy()); 
    ...
    String deliStartTime	= m.getParameter("deliStartTime");
    String deliEndTime	= m.getParameter("deliEndTime");
    ...
    String storedFile=dir+"\\"+filename;	//저장 경로 + 파일 명(DB에 이대로 저장 됨)
        ...
        String RestaurantNo=(String)session.getAttribute("RestaurantNo"); 
        String res=(String)session.getAttribute("en_mo");			
    	session.removeAttribute("pre_priority");
    	session.removeAttribute("RestaurantNo");
    	session.removeAttribute("en_mo");
    	
        if(res.equals("en")){			//등록 과정인지 수정 과정인지 구분
        	rst = dao.insertRestaurant(title, priority, content, address, deliStartTime, deliEndTime, storedFile);
        }else{
        	
	    	rst = dao.updateRestaurant(pre_priority, RestaurantNo, title, priority, content, address, deliStartTime, deliEndTime, storedFile);
        }
