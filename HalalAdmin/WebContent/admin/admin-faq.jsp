...
<%
	...
	list = dao.FAQInfo();									//FAQ정보 가져옴

	String s=null;
	s=(String)session.getAttribute("priority");				//FAQ 우선순위 변경 후 돌아 왔다면 s값이 null이 아님
 ...
 if(num_temp==null){
	 ...
	 var num = $('input[name=chk_pop]:checked').val();			// 라디오 박스 체크 확인  name값이 radio name
	  if(num==null){
		 ...
		 document.form.action="faq-rm.jsp";
...
//Faq 새로 등록
function enroll(){
		document.form.action="faq-enroll.jsp"
		document.form.submit();
	 ...
	 if(num_temp==null){
		 ...
		 var num = $('input[name=chk_pop]:checked').val();				// 라디오 박스 체크 확인  name값이 radio name
		  ...
		  alert("수정 할 항목을 선택하세요"); 
					...
					document.form.action="faq-modify.jsp"
					document.form.submit();
			}
	 }
...
function up(){
	var num = $('input[name=chk_pop]:checked').val();			// 라디오 박스 체크 확인  name값이 radio name
	  if(num==null){
		 ...
		 document.form.action="faq-Upproc.jsp";
		 document.form.submit();
	 }
...
function down(){
	var num = $('input[name=chk_pop]:checked').val();			// 라디오 박스 체크 확인  name값이 radio name
	  if(num==null){
		 ...
		 document.form.action="faq-Downproc.jsp";
			document.form.submit();
	 }
										...
										<tbody>
											...
											<tr>
												<td width=50 align ="center"><input type="radio" name="chk_pop"
													value="<%=vo.getFaqPriority()%>" /></td>
												<td width=60 align ="center"><%=vo.getFaqPriority()%></td>
												<td><%=vo.getFaqQuestion()%></td>
											</tr>
											<% 
												} 
											}else{
												for(adminVo vo : list){ 
													if(vo.getFaqPriority().equals(s))	//우선 순위 변경 후 Radio Box 계속 체크 되어 있게 하기 위함
													{
														
													%>
														<tr>
														<td width=50 align ="center"><input type="radio" name="chk_pop"
															value="<%=vo.getFaqPriority()%>"  checked/></td>
														<td width=60 align ="center"><%=vo.getFaqPriority()%></td>
														<td><%=vo.getFaqQuestion()%></td>
													</tr>
													<%
